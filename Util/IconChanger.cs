...
namespace ThaumaStudio.Util
{
    class IconChanger
    {
        ...
        {
            // Load executable
            IntPtr handleExe = BeginUpdateResource(exeFilePath, true);
            ...
            foreach (var icon in icons)
            {
                ...
                ret = UpdateResource(handleExe, RT_ICON, index, 1033, icon.Data, icon.Size);
            ...
            // todo :Improve the return value handling of UpdateResource
            ret = UpdateResource(handleExe, RT_GROUP_ICON, 104, 1033, groupdata, (uint)groupdata.Length);
            if (ret == 1)
            {
        ...
        public static void RefreshExplorerIcons()
        {
            ...
            startInfo.Arguments = "/C ie4uinit.exe -show";
